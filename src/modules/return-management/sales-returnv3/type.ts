// src/modules/sales-returnv3/type.ts
  ...
  export interface SalesReturnItem {
    id?: number;          // 🟢 NEW: Stores the row ID (useful for editing/deleting specific rows)
  ...
  // --- BASIC ENTITIES ---
  export interface Brand { brand_id: number; brand_name: string; }
  export interface Category { category_id: number; category_name: string; }
  export interface Supplier { id: number; supplier_name: string; }
...
// ... rest of the file ...
  ...
  export interface ProductSupplierConnection { id: number; supplier_id: number; product_id: number; }

  // --- API LOOKUPS ---
  export interface API_DiscountType { id: number; discount_type: string; }
  ...
  export interface API_SalesReturnType { type_id: number; type_name: string; description: string; }
