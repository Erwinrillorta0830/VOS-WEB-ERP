...
import { X, Plus, Trash2, Save, ChevronDown, AlertCircle, FileText, User, Calculator, CheckCircle } from "lucide-react"; 
...
// 🟢 UPDATE: Added InvoiceOption to imports
import { SalesReturnItem, API_LineDiscount, API_SalesReturnType, InvoiceOption } from "../type";

// Import Child Modal
import { ProductLookupModal } from "./ProductLookupModal";
// Import Provider & Types
import { SalesReturnProvider, SalesmanOption, CustomerOption, BranchOption } from "../provider/api";
...
export function CreateSalesReturnModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  // Default return type logic
  const [defaultReturnType] = useState("Good Order");
  ...
  const [lineDiscountOptions, setLineDiscountOptions] = useState<API_LineDiscount[]>([]);
  const [returnTypeOptions, setReturnTypeOptions] = useState<API_SalesReturnType[]>([]);

  // 🟢 UPDATE: INVOICE DATA LIST & DROPDOWN STATE
  ...
  useEffect(() => {
    if (isOpen) {
      const loadData = async () => {
        try {
          // 🟢 UPDATE: Added getInvoiceReturnList to the fetch list
          const [salesmenData, customersData, branchesData, lineDiscountData, returnTypesData, invoiceListData] = await Promise.all([
            ...
            SalesReturnProvider.getInvoiceReturnList() // <--- NEW API CALL
          ]);
          ...
          setInvoiceOptions(invoiceListData); // <--- SET INVOICE DATA
        } catch (error) {
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      // 🟢 UPDATE: Invoice
      if (invoiceWrapperRef.current && !invoiceWrapperRef.current.contains(target)) {
  ...
  // 🟢 UPDATE: Filter Logic for Invoices
  ...
  const handleOpenProductLookup = () => {
    setValidationError(null);
    if (!returnDate) { setValidationError("Please select a Return Date before adding products."); return; }
    if (!selectedSalesmanId) { setValidationError("Please select a Salesman before adding products."); return; }
    if (!selectedCustomerId) { setValidationError("Please select a Customer before adding products."); return; }
    setIsProductLookupOpen(true);
  };

  const handleCreateReturn = async () => {
    setValidationError(null);
    if (!returnDate) { setValidationError("Return Date is required."); return; }
    if (items.length === 0) { setValidationError("Please add at least one product."); return; }
    ...
    try {
      ...
      const payload = {
        invoiceNo, // Uses the state from the new dropdown
        orderNo,
    ...
    setItems(prev => {
        ...
        if (field === 'discountType') {
            if (value === "") {
                ...
                const selectedOption = lineDiscountOptions.find(d => d.id.toString() === value.toString());
                if (selectedOption) {
                    const percentage = parseFloat(selectedOption.percentage) || 0;
                    item.discountAmount = (item.grossAmount || 0) * (percentage / 100);
                }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-2 md:p-4 animate-in fade-in duration-300">
      ...
      <div className="bg-slate-50 w-full h-full md:max-w-[1300px] md:h-[95vh] md:rounded-xl shadow-2xl flex flex-col overflow-hidden ring-1 ring-white/20 animate-in zoom-in-95 duration-300 ease-out">
        ...
        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200 bg-white">
          <div className="flex items-center gap-3">
            ...
            <div>
                <h2 className="text-lg font-bold text-gray-800">Create Sales Return</h2>
                <p className="text-xs text-gray-500">Fill in the details below to process a return</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
          ...
          {validationError && (
            <div className="p-4 bg-red-50 border-l-4 border-red-500 text-red-700 flex items-center justify-between rounded-r shadow-sm animate-in fade-in slide-in-from-top-2 duration-300 ease-out">
              ...
              <button onClick={() => setValidationError(null)} className="bg-red-500 hover:bg-red-600 text-white h-7 w-7 rounded-md flex items-center justify-center shadow-sm transition-all active:scale-95" title="Dismiss">
                <X className="h-4 w-4" />
              </button>
            </div>
          )}
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div className="space-y-4">
                    <div className="space-y-1.5 relative" ref={salesmanWrapperRef}>
                        <label className="text-[11px] font-bold text-gray-500 uppercase tracking-wide">Salesman <span className="text-red-500">*</span></label>
                        <div className="relative group">
                            ...
                            <input
                                ...
                                onFocus={() => { setIsSalesmanOpen(true); setSalesmanSearch(""); }}
                            />
                            ...
                            <div className="absolute top-[calc(100%+4px)] left-0 w-full z-20 bg-white border border-gray-200 rounded-md shadow-xl max-h-60 overflow-y-auto animate-in slide-in-from-top-2 fade-in duration-200">
                                {filteredSalesmen.length > 0 ? (
                                    filteredSalesmen.map((s) => (
                                        <div key={s.id} className={`px-4 py-2.5 text-sm cursor-pointer hover:bg-blue-50 transition-colors duration-150 ${selectedSalesmanId === s.id.toString() ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-700'}`} onClick={() => handleSelectSalesman(s)}>
                                            {s.name}
                                        </div>
                                    ))
                                ) : <div className="px-4 py-3 text-sm text-gray-400 text-center">No salesman found</div>}
                            </div>
                        )}
                    </div>
                    <div className="space-y-1.5">
                        <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wide">Salesman Code</label>
                        <Input value={salesmanCode} readOnly className="h-10 bg-slate-50 border-gray-200 text-gray-500 font-mono text-xs transition-all duration-200 focus:bg-white" />
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    <div className="space-y-1.5 relative" ref={customerWrapperRef}>
                        <label className="text-[11px] font-bold text-gray-500 uppercase tracking-wide">Customer <span className="text-red-500">*</span></label>
                        <div className="relative group">
                            <User className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400 group-focus-within:text-blue-500 transition-colors duration-200"/>
                            <input
                                type="text"
                                ...
                                onFocus={() => { setIsCustomerOpen(true); setCustomerSearch(""); }}
                            />
                            <ChevronDown className="h-4 w-4 text-gray-400 absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none" />
                        </div>
                        {isCustomerOpen && (
                            <div className="absolute top-[calc(100%+4px)] left-0 w-full z-20 bg-white border border-gray-200 rounded-md shadow-xl max-h-60 overflow-y-auto animate-in slide-in-from-top-2 fade-in duration-200">
                                ...
                                ) : <div className="px-4 py-3 text-sm text-gray-400 text-center">No customer found</div>}
                            </div>
                        )}
                    </div>
                    <div className="space-y-1.5">
                        <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wide">Customer Code</label>
                        <Input value={customerCode} readOnly className="h-10 bg-slate-50 border-gray-200 text-gray-500 font-mono text-xs transition-all duration-200 focus:bg-white" />
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    <div className="space-y-1.5">
                        <label className="text-[11px] font-bold text-gray-500 uppercase tracking-wide">Return Date <span className="text-red-500">*</span></label>
                        <Input
                            type="date"
                            value={returnDate}
                            onChange={(e) => { setReturnDate(e.target.value); setValidationError(null); }}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1.5">
                            ...
                            <Input value={branchName} readOnly className="h-10 bg-slate-50 border-gray-200 text-gray-500 text-xs transition-all duration-200" />
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-[11px] font-bold text-gray-500 uppercase tracking-wide">Price Type</label>
                    ...
                    <div className="flex items-center pt-2">
                        <Checkbox id="thirdParty" checked={isThirdParty} onCheckedChange={(c) => setIsThirdParty(c as boolean)} className="data-[state=checked]:bg-blue-600 border-gray-300 transition-all duration-200" />
                        <label htmlFor="thirdParty" className="ml-2 text-sm font-medium text-gray-600 cursor-pointer select-none">Third Party Transaction</label>
                    </div>
                </div>
             </div>
          </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-shadow duration-300">
              <div className="flex justify-between items-center px-5 py-4 bg-white border-b border-gray-100">
                  <h3 className="font-bold text-gray-800 flex items-center gap-2">
                      <div className="bg-blue-100 p-1.5 rounded text-blue-600"><Calculator className="h-4 w-4"/></div>
                      Products Summary
                  </h3>
                  <Button size="sm" onClick={handleOpenProductLookup} className="bg-blue-600 hover:bg-blue-700 text-white shadow-blue-200 shadow-md active:scale-95 transition-all duration-200 ease-in-out">
                  ...
                  <table className="w-full text-sm text-left min-w-[1100px]">
                      <thead>
                          <tr className="bg-blue-600 text-white">
                              <th className="px-4 py-3 font-semibold text-xs uppercase tracking-wider w-28">Code</th>
                              <th className="px-4 py-3 font-semibold text-xs uppercase tracking-wider">Description</th>
                              <th className="px-4 py-3 font-semibold text-xs uppercase tracking-wider w-20">Unit</th>
                              <th className="px-4 py-3 font-semibold text-xs uppercase tracking-wider w-24 text-center">Qty</th>
                              <th className="px-4 py-3 font-semibold text-xs uppercase tracking-wider w-28 text-right">Price</th>
                              <th className="px-4 py-3 font-semibold text-xs uppercase tracking-wider w-28 text-right">Gross</th>
                              <th className="px-4 py-3 font-semibold text-xs uppercase tracking-wider w-36">Disc. Type</th>
                              <th className="px-4 py-3 font-semibold text-xs uppercase tracking-wider w-24 text-right">Disc.</th>
                              <th className="px-4 py-3 font-semibold text-xs uppercase tracking-wider w-32 text-right">Total</th>
                              <th className="px-4 py-3 font-semibold text-xs uppercase tracking-wider w-32">Reason</th>
                              <th className="px-4 py-3 font-semibold text-xs uppercase tracking-wider w-32">Type</th>
                      ...
                      <tbody className="divide-y divide-gray-100">
                          {items.length === 0 ? (
                              <tr>
                                  <td colSpan={12} className="px-6 py-16 text-center text-gray-400 bg-slate-50">
                                      <div className="flex flex-col items-center gap-2 animate-in fade-in duration-500">
                                        <FileText className="h-8 w-8 text-gray-300 mb-1" />
                                        <p>No items added yet.</p>
                                        <span className="text-xs">Click "Add Product" to browse catalog.</span>
                                      </div>
                                  </td>
                              </tr>
                          ) : (
                              items.map((item, idx) => (
                                  <tr key={idx} className="hover:bg-blue-50/50 group transition-colors duration-200 ease-in-out">
                                      ...
                                      <td className="px-4 py-2">
                                          <span className="bg-gray-100 text-gray-600 px-2 py-0.5 rounded text-xs border border-gray-200">{item.unit}</span>
                                      </td>
                                      <td className="px-4 py-2">
                                          <input 
                                            type="number" min="1"
                                      ...
                                      <td className="px-4 py-2 text-right text-gray-500">₱{(item.grossAmount || 0).toLocaleString()}</td>
                                      <td className="px-4 py-2">
                                          <select 
                                          ...
                                          >
                                            ...
                                            {lineDiscountOptions.map((opt) => (
                                                <option key={opt.id} value={opt.id}>{opt.line_discount}</option>
                                            ))}
                                          </select>
                                      </td>
                                      <td className="px-4 py-2">
                                          <input 
                                            ...
                                            className="w-full text-right border border-gray-300 rounded h-8 text-sm text-red-500 focus:border-blue-500 outline-none transition-all duration-200"
                                      ...
                                      <td className="px-4 py-2 text-right font-bold text-blue-700">₱{item.totalAmount.toLocaleString()}</td>
                                      <td className="px-4 py-2">
                                          <select 
                                            className="w-full border border-gray-300 rounded h-8 text-xs px-1 bg-white focus:border-blue-500 outline-none transition-all duration-200"
                                            value={item.reason} onChange={(e) => handleItemChange(idx, "reason", e.target.value)}
                                          >
                                            <option>Good Order</option>
                                      ...
                                      <td className="px-4 py-2">
                                          <select 
                                            className="w-full border border-gray-300 rounded h-8 text-xs px-1 bg-white focus:border-blue-500 outline-none transition-all duration-200"
                                            value={item.returnType} onChange={(e) => handleItemChange(idx, "returnType", e.target.value)}
                                          >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-20">
              <div className="space-y-4 bg-white p-5 rounded-lg border border-gray-200 shadow-sm h-full hover:shadow-md transition-shadow duration-300">
                  <h4 className="font-bold text-gray-700 text-sm mb-2">Additional Information</h4>
                  <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                          <label className="text-[11px] font-bold text-gray-500 uppercase tracking-wide">Order No. (Optional)</label>
                          <Input value={orderNo} onChange={(e) => setOrderNo(e.target.value)} className="h-9 border-gray-300 transition-all duration-200 focus:bg-white" placeholder="e.g. ORD-001" />
                      </div>

                      {/* 🟢 UPDATE: INVOICE NO DROPDOWN */}
                      <div className="space-y-1.5" ref={invoiceWrapperRef}>
                          <label className="text-[11px] font-bold text-gray-500 uppercase tracking-wide">Invoice No. (Optional)</label>
                          <div className="relative group">
                              <input
                                  type="text"
                                  ...
                                  value={invoiceSearch || invoiceNo} // Show selected or what is being typed
                                  onChange={(e) => {
                                      setInvoiceSearch(e.target.value);
                                      setInvoiceNo(e.target.value); // Allow free typing too
                              ...
                              {isInvoiceOpen && (
                                  <div className="absolute bottom-[calc(100%+4px)] left-0 w-full z-50 bg-white border border-gray-200 rounded-md shadow-xl max-h-48 overflow-y-auto animate-in slide-in-from-bottom-2 fade-in duration-200">
                                      {filteredInvoices.length > 0 ? (
                                          filteredInvoices.map((inv) => (
                                              <div 
                                                  ...
                                                  onClick={() => {
                                                      ...
                                                      setInvoiceSearch(inv.invoice_no); // Sync search text
                                                      setIsInvoiceOpen(false);
                                                  }}
                                              >
                                                  {inv.invoice_no}
                                          ...
                                          <div className="px-3 py-2 text-xs text-gray-400 text-center">No invoices found</div>
                                      )}
                                  </div>
                              )}
                          </div>
                  ...
                  <div className="space-y-1.5">
                      <label className="text-[11px] font-bold text-gray-500 uppercase tracking-wide">Remarks</label>
                      <Textarea value={remarks} onChange={(e) => setRemarks(e.target.value)} className="resize-none h-24 border-gray-300 focus:border-blue-500 transition-all duration-200 focus:bg-white" placeholder="Add any notes regarding this return..." />
                  </div>
              </div>
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-0 shadow-sm overflow-hidden h-fit hover:shadow-md transition-shadow duration-300">
                  ...
                  <div className="p-6 space-y-4">
                      <div className="flex justify-between items-center text-sm text-gray-600">
                          ...
                          <span className="font-medium text-gray-900 tabular-nums">₱{totalGross.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
                      </div>
                      <div className="flex justify-between items-center text-sm text-red-500">
                          <span>Total Discount</span>
                          <span className="font-medium tabular-nums">- ₱{totalDiscount.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
                      </div>
                      ...
                      <div className="flex justify-between items-center">
                          ...
                          <span className="text-2xl font-bold text-blue-600 tabular-nums">₱{totalNet.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
                      </div>
                  </div>
              </div>
          </div>
        ...
        <div className="p-4 bg-white border-t border-gray-200 flex justify-end gap-3 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
            <Button variant="outline" onClick={handleClose} className="h-11 px-6 border-gray-300 text-gray-700 hover:bg-gray-50 font-medium active:scale-95 transition-all duration-200">Cancel</Button>
            <Button onClick={handleCreateReturn} className="h-11 px-8 bg-blue-600 hover:bg-blue-700 text-white font-semibold shadow-blue-200 shadow-lg active:scale-95 transition-all duration-200 ease-in-out">
      ...
      <ProductLookupModal isOpen={isProductLookupOpen} onClose={() => setIsProductLookupOpen(false)} onConfirm={handleAddProducts}
       />
       {/* SUCCESS MODAL */}
      <Dialog open={isSuccessOpen} onOpenChange={(open) => !open && handleFinalize()}>
