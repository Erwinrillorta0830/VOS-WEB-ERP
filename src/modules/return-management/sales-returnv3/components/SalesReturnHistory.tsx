...
export function SalesReturnHistory() {
  ...
  const handleAddProductsToEdit = (newItems: any[]) => {
    ...
    const formattedItems = newItems.map((item, index) => {
      ...
      const uniqueId =
        item.tempId ||
        `added-${Date.now()}-${index}-${Math.random().toString(36).substr(2, 9)}`;
  ...
  const getSalesmanBranch = (id: any) => {
    if (!id) return "-";
    const salesman = salesmenOptions.find(
      (opt) => String(opt.value) === String(id),
    );
  ...
  const getCustomerName = (code: any) => {
    if (!code) return "-";
    const found = customerOptions.find(
      (opt) => String(opt.value) === String(code),
    );
  ...
  return (
    <div className="space-y-6 p-4 md:p-8 w-full max-w-[100vw] overflow-x-hidden bg-slate-50 min-h-screen">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 items-end">
          ...
          <div className="space-y-1.5 relative" ref={salesmanWrapperRef}>
            ...
            {isSalesmanOpen && (
              <div className="absolute top-[calc(100%+4px)] left-0 w-full z-50 bg-white border border-slate-200 rounded-lg shadow-xl max-h-60 overflow-y-auto ring-1 ring-slate-200">
                <div
                  className={`px-4 py-2.5 text-sm cursor-pointer hover:bg-blue-50 transition-colors ${selectedSalesman === "All" ? "bg-blue-50 text-blue-700 font-medium" : "text-slate-600"}`}
          ...
          <div className="space-y-1.5 relative" ref={customerWrapperRef}>
            ...
            {isCustomerOpen && (
              <div className="absolute top-[calc(100%+4px)] left-0 w-full z-50 bg-white border border-slate-200 rounded-lg shadow-xl max-h-60 overflow-y-auto ring-1 ring-slate-200">
                <div
                  className={`px-4 py-2.5 text-sm cursor-pointer hover:bg-blue-50 transition-colors ${selectedCustomer === "All" ? "bg-blue-50 text-blue-700 font-medium" : "text-slate-600"}`}
          ...
          <div className="pt-2 sm:pt-0">
            <Button
              variant="outline"
              onClick={handleReset}
              className="w-full h-10 border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-blue-600 hover:border-blue-200 transition-colors"
            >
          ...
          <Table className="min-w-[1000px]">
            <TableHeader className="bg-slate-50/80">
              <TableRow className="border-b-slate-200 hover:bg-slate-50/80">
                <TableHead className="font-bold text-blue-900 whitespace-nowrap pl-6">
                  Return No.
                </TableHead>
                <TableHead className="font-bold text-blue-900 whitespace-nowrap">
                  Salesman
                </TableHead>
                <TableHead className="font-bold text-blue-900 whitespace-nowrap">
                  Customer
                ...
                <TableHead className="font-bold text-blue-900 text-right whitespace-nowrap">
                  Total Amount
                </TableHead>
                <TableHead className="font-bold text-blue-900 text-center whitespace-nowrap">
                  Status
                </TableHead>
                <TableHead className="font-bold text-blue-900 whitespace-nowrap">
                  Remarks
                </TableHead>
                {/* 🔴 REMOVED ACTION COLUMN HEADER HERE */}
              </TableRow>
            </TableHeader>
            <TableBody>
              {loading ? (
                ...
                data.map((row) => (
                  ...
                  >
                    ...
                    {/* 🔴 REMOVED ACTION CELL BUTTON HERE */}
                  </TableRow>
                ))
              )}
            </TableBody>
        ...
        <div className="flex items-center gap-2 order-1 sm:order-2 mb-2 sm:mb-0">
          <div className="flex gap-1">
            <Button
              variant="outline"
              size="icon"
              className="h-8 w-8 hidden sm:flex border-slate-200 text-slate-600 hover:text-blue-600 hover:border-blue-200"
      ...
      >
        <DialogContent className="w-full max-w-[95vw] lg:max-w-7xl max-h-[92vh] flex flex-col p-0 overflow-hidden bg-white border-0 shadow-2xl rounded-xl">
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-blue-600 hover:bg-blue-700 border-none">
                        <TableHead className="text-white font-semibold h-11 w-[100px]">
                          Code
                        </TableHead>
                        <TableHead className="text-white font-semibold h-11 min-w-[150px]">
                          Description
                        </TableHead>
                        <TableHead className="text-white font-semibold h-11">
                          Unit
                        </TableHead>
                        <TableHead className="text-white font-semibold h-11 text-center w-[80px]">
                          Qty
                        </TableHead>
                        <TableHead className="text-white font-semibold h-11 text-right w-[100px]">
                          Price
                        </TableHead>
                        <TableHead className="text-white font-semibold h-11 text-right">
                          Gross
                        </TableHead>
                        <TableHead className="text-white font-semibold h-11 w-[140px]">
                          Disc. Type
                        </TableHead>
                        <TableHead className="text-white font-semibold h-11 text-right w-[100px]">
                          Disc. Amt
                    ...
                    <TableBody>
                      {detailsLoading ? (
                        ...
                        details.map((item, i) => (
                          <TableRow
                            key={i}
                            className="hover:bg-blue-50/30 border-b-slate-100"
                          >
                            <TableCell className="text-xs text-slate-700 font-bold align-middle">
                              {item.code || "N/A"}
                            </TableCell>
                            <TableCell className="text-xs text-slate-700 align-middle font-medium">
                              {item.description || "-"}
                            </TableCell>
                            ...
                            <TableCell className="text-center align-middle p-2">
                              {isEditable ? (
                                <Input
                                  type="number"
                                  className="h-8 text-center text-xs border-slate-200 focus:border-blue-500 bg-slate-50 focus:bg-white"
                                  value={item.quantity}
                                  onChange={(e) =>
                                    handleDetailChange(
                                      i,
                            ...
                            <TableCell className="text-right align-middle p-2">
                              {isEditable ? (
                                <Input
                                  type="number"
                                  className="h-8 text-right text-xs border-slate-200 focus:border-blue-500 bg-slate-50 focus:bg-white"
                            ...
                            <TableCell className="align-middle p-2">
                              {isEditable ? (
                                <Input
                                  className="h-8 text-xs border-slate-200 bg-slate-50 focus:bg-white"
                                  value={item.reason}
                                  onChange={(e) =>
                                    handleDetailChange(
                                      i,
                              ...
                              {isEditable ? (
                                ...
                                >
                                  ...
                                  <SelectContent>
                                    ...
                                    <SelectItem value="Cylinder Exchange">
                                      Cylinder Exchange
                                    </SelectItem>
                                    <SelectItem value="Pull Out">
                                      Pull Out
                                    </SelectItem>
                                  </SelectContent>
                                </Select>
                ...
                <div className="bg-white p-6 rounded-xl border border-blue-100 shadow-sm space-y-3 relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-semibold text-slate-800">
                      ...
                      {details
                        .reduce(
                          (acc, i) =>
                            acc + (i.grossAmount || i.quantity * i.unitPrice),
                          0,
                        )
                  ...
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-semibold text-slate-800">
                      ₱
                      {details
                        .reduce((acc, i) => acc + (i.discountAmount || 0), 0)
                        .toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
          ...
          <div className="border-t border-slate-100 p-5 bg-white flex justify-end gap-3 sticky bottom-0 z-20">
            ...
            <Button
              variant="outline"
              className="bg-white text-slate-700 border-slate-300 min-w-[90px] hover:bg-slate-50"
              onClick={() => setSelectedReturn(null)}
            >
              Close
            </Button>
            ...
            {isEditable && (
              <>
                ...
                <Button
                  className="bg-blue-600 hover:bg-blue-700 text-white min-w-[160px] shadow-md shadow-blue-200"
          ...
          <div className="flex flex-col items-center text-center gap-4">
            ...
            <div className="space-y-2">
              ...
              <div className="text-sm text-slate-500 leading-relaxed">
                ...
                as <span className="font-bold text-green-600">RECEIVED</span>?
              </div>
            </div>
          </div>
      ...
      <div className="relative z-[9999]">
        {" "}
        {/* Wrapped in high Z-index to force display on top */}
        <ProductLookupModal
