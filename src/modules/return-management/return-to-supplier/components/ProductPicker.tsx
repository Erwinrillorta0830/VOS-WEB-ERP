...
import { Search, Scan, X, Plus, Minus, Package, Trash2, ShoppingCart, AlertCircle } from "lucide-react";
...
export function ProductPicker({
  ...
  const filteredRawProducts = useMemo(() => {
    ...
    if (barcodeQuery) {
        ...
        result = result.filter(p => p.code.toLowerCase().includes(lowerBarcode)); 
    }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 h-full animate-in fade-in slide-in-from-right-8 duration-500 ease-out relative bg-white lg:bg-slate-50">
      ...
      <div className="col-span-1 lg:col-span-2 flex flex-col h-full bg-slate-50/50 border-r border-slate-200 overflow-hidden relative">
        ...
        {/* SCROLLABLE GRID */}
        <div className="flex-1 overflow-y-auto p-4 lg:p-6 custom-scrollbar pb-24 lg:pb-6 relative bg-slate-50/50">
                    ...
                    >
                      ...
                      <div className="p-4 border-b border-slate-50 bg-slate-50/30">
                          ...
                          <div className="flex items-center gap-2 mt-1.5">
                              ...
                              <span className="text-xs font-medium text-slate-700 font-mono">{mainProduct.code}</span>
                          </div>
                      </div>
                      ...
                      <div className="p-4 space-y-3 bg-white flex-1">
                        {group.map((variant) => {
                            // ✅ NEW: Force mock stock to 10 for display
                            const displayStock = variant.stock || 10;

                            return (
                                <div key={variant.id} className="flex items-center justify-between group/item">
                                    
                                    {/* Left Side: Price & Unit */}
                                    <div className="flex flex-col">
                                        <div className="flex items-baseline gap-1.5">
                                                <span className="text-lg font-bold text-slate-900">
                                                    ₱{variant.price.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                                </span>
                                        </div>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex flex-col items-end mr-1">
                                            ...
                                            <span className={`text-sm font-bold tabular-nums leading-none ${
                                                displayStock > 0 ? "text-emerald-600" : "text-red-500"
                                            }`}>
                                                {displayStock}
                                            </span>
                                        </div>
                                        ...
                                        <Button 
                                            size="sm"
                                            // ✅ UPDATED: Use the new handleAddToSelection
      ...
      {/* ========================================================= */}
      <div className="hidden lg:flex col-span-1 bg-gray-50 flex-col h-full shadow-[-1px_0_0_0_rgba(0,0,0,0.05)] z-20 overflow-hidden">
        
        {/* Header */}
        <div className="p-5 lg:p-6 border-b border-gray-200 bg-white z-10">
              ...
              >
                ...
                <div className="flex justify-between items-center bg-gray-50 p-2 rounded-lg border border-gray-100">
                  <div className="flex items-center gap-1 bg-white rounded-md border border-gray-200 shadow-sm p-0.5">
                    ...
                    <span className="text-xs font-bold w-10 text-center tabular-nums text-gray-900">{item.quantity}</span>
                    <Button 
                        variant="ghost" 
                        size="icon" 
                        className="h-7 w-7 hover:bg-gray-100 rounded text-gray-600" 
                  ...
                  <span className="text-sm font-bold text-gray-900 tabular-nums pr-1">
                    ₱{(item.quantity * (item.customPrice || item.price)).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                  </span>
                </div>
              </div>
            ))
        ...
        {/* Footer */}
        <div className="p-5 border-t border-gray-200 bg-white shadow-[0_-4px_20px_-5px_rgba(0,0,0,0.05)] z-30 space-y-4 flex-shrink-0">
      ...
      <div className="lg:hidden absolute bottom-0 left-0 w-full bg-white border-t border-slate-200 p-4 z-50 flex items-center justify-between shadow-[0_-4px_20px_rgba(0,0,0,0.1)]">
        <div className="flex flex-col">
            <span className="text-xs text-slate-500 font-medium">Total ({addedProducts.length} items)</span>
            <span className="text-xl font-bold text-blue-600">
                ₱{currentTotal.toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </span>
        </div>
        <Button onClick={onClose} className="bg-blue-600 text-white px-6 font-bold shadow-md rounded-lg h-11">
            Confirm
        </Button>
      </div>
    </div>
