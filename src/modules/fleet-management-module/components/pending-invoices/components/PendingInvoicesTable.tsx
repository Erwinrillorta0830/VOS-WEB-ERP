...
export function PendingInvoicesTable({
  ...
  // ✅ Calculate RowSpans AND Total Amounts for Dispatch Plan Column
  const { spanMap, hiddenIndices, sumMap } = React.useMemo(() => {
    const spanMap = new Map<number, number>();
    const sumMap = new Map<number, number>(); // Stores the total amount for the group
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Do NOT merge "unlinked" or empty items
      ...
      for (let j = i + 1; j < rows.length; j++) {
        if (rows[j].dispatch_plan === currentPlan) {
          span++;
          totalAmount += (rows[j].net_amount || 0); // Aggregate amount
          hiddenIndices.add(j);
        } else {
          break;
        }
      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-slate-50 hover:bg-slate-50">
            ...
            <TableHead className="w-[140px] text-center text-xs font-bold text-slate-500 uppercase">
              NET AMOUNT
            </TableHead>
            <TableHead className="w-[180px] text-center text-xs font-bold text-slate-500 uppercase">
              DISPATCH PLAN
            </TableHead>
        ...
        <TableBody>
          {rows.map((r, index) => {
            ...
            return (
              <TableRow key={r.id || r.invoice_no} className="hover:bg-slate-50/80">
                <TableCell>
                  <button
                ...
                {/* ✅ Dispatch Plan Column with Total Amount */}
                {!isHidden && (
                    <TableCell 
                        rowSpan={rowSpan} 
                        className={`text-xs text-slate-500 align-middle text-center ${rowSpan && rowSpan > 1 ? 'bg-white/50 border-b border-l border-r border-slate-100' : ''}`}
                        style={{ verticalAlign: 'middle' }}
                    >
                        {r.dispatch_plan && r.dispatch_plan !== "unlinked" ? (
                            <div className="flex flex-col gap-1">
                                <span className="font-medium text-slate-700">{r.dispatch_plan}</span>
                                {/* Display Total Amount if calculated (only if > 0) */}
