...
import { Truck, Package, MapPin, LayoutDashboard, Fuel, Wrench, UserCheck, ClipboardList, BarChart3, Menu, X, Users, Route, AlertTriangle, FileText, Settings, Shield, Calendar, ClipboardCheck, PackageCheck } from 'lucide-react';
...
export function Layout({ currentPage, onNavigate, children }: LayoutProps) {
  ...
  return (
    ...
    style={{ backdropFilter: 'blur(4px)' }}>
      ...
      {/* Sidebar */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 flex flex-col transform transition-transform duration-300 lg:transform-none ${
        ...
        <nav className="flex-1 overflow-y-auto p-4">
          {menuItems.map((section) => (
            <div key={section.category} className="mb-6">
              <h3 className="text-xs uppercase text-gray-500 px-3 mb-2">{section.category}</h3>
