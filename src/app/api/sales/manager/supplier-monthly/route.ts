...
export async function GET() {
    try {
        // Use consistent exclusions (avoid parsing issues)
        const excludedDivisions = ["Internal", "Internal Division"].join(",");

        const [salesJson, productsJson, ppsJson, suppliersJson] = await Promise.all([
            directusFetch<DirectusJson<any[]>>(
                `/items/sales?fields=${encodeURIComponent("product_id,amount,date,division")}` +
                `&filter[division][_nin]=${encodeURIComponent(excludedDivisions)}` +
                `&limit=-1`
            ),
            directusFetch<DirectusJson<any[]>>(`/items/products?limit=-1&fields=${encodeURIComponent("product_id,parent_id")}`),
            directusFetch<DirectusJson<any[]>>(
                `/items/product_per_supplier?limit=-1&fields=${encodeURIComponent("product_id,supplier_id")}`
            ),
            directusFetch<DirectusJson<any[]>>(`/items/suppliers?limit=-1&fields=${encodeURIComponent("id,supplier_name")}`),
        ]);
        ...
        // product -> supplier_id (first wins)
        ...
        const supplierNameMap = new Map<string, string>(
            suppliers.map((s: any) => [String(s.id), String(s.supplier_name || "").toUpperCase()])
        );

        // resolve root (family) id
        ...
        // root -> supplier frequency (inherit supplier from descendants)
        ...
        // pick best supplier per root (most frequent), then override if root has direct supplier
        ...
        const formattedData = sales
            .map((item: any) => {
                const aligned = resolveSupplierNameOrNull(item.product_id);
                if (!aligned) return null;
                return {
                    ...item,
                    display_name: aligned,
                    aligned_supplier: aligned,
                };
            })
        ...
        return NextResponse.json({
            success: true,
            data: {
                suppliers: [...new Set(formattedData.map((i) => i.display_name))],
            ...
            _debug: {
                ...
                counts: {
                    ...
                    formatted: formattedData.length,
                },
            },
        });
    } catch (error: any) {
        console.error("❌ contribution route error:", error);
        return NextResponse.json(
            {
