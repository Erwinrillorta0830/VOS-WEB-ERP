// app/(app)/sales/encoder/page.tsx
"use client";
      ...
      <span className="text-xs font-medium w-16 text-right">
        {formatCurrency(gap)}
            ...
            <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                <div
                    className={`h-full ${isPositiveGap ? "bg-black" : "bg-green-500"} rounded-full`}
...
export default function SalesmanDashboard() {
    ...
    useEffect(() => {
        async function fetchSalesmen() {
            try {
                const res = await fetch("/api/sales/encoder?type=salesmen", { cache: "no-store" });
                if (!res.ok) {
                    const text = await res.text();
                    console.error("Salesmen API failed:", res.status, text);
                    return;
                }
    ...
    useEffect(() => {
        ...
        async function fetchDashboard() {
            ...
            try {
                // CRITICAL FIX: must be absolute /api path, not ../api
                const res = await fetch(
                    `/api/sales/encoder?type=dashboard&salesmanId=${encodeURIComponent(
                        selectedSalesman
                    )}`,
                    { cache: "no-store" }
                );
    ...
    const totalReturnPages = data ? Math.ceil(data.returnHistory.length / itemsPerPage) : 0;

    const currentReturns = data
        ? data.returnHistory.slice((returnPage - 1) * itemsPerPage, returnPage * itemsPerPage)
        : [];
    ...
    const totalProductPages = data ? Math.ceil(data.topProducts.length / itemsPerPage) : 0;

    const currentProducts = data
        ? data.topProducts.slice((productPage - 1) * itemsPerPage, productPage * itemsPerPage)
        : [];
        ...
        <div className="p-6 max-w-[1600px] mx-auto space-y-8 bg-gray-50/30 min-h-screen">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground">Personal sales performance and order tracking</p>
                ...
                <div className="flex items-center gap-2 bg-white p-2 rounded-lg border shadow-sm">
                    <span className="text-sm font-semibold text-gray-700 ml-2">Salesman:</span>
                    <select
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <KPICard title="Total Orders" value={data.kpi.totalOrders} icon={ShoppingCart} subtitle="All time" />
                <KPICard
                    title="Order Value"
                ...
                />
                <KPICard title="Pending Orders" value={data.kpi.pendingOrders} icon={Package} subtitle="Awaiting fulfillment" />
                <KPICard title="Returns" value={data.kpi.returns} icon={RefreshCcw} subtitle="Total returns" />
            </div>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Monitor your performance against targets</CardDescription>
                </CardHeader>
                ...
                In Progress
                        ...
                        <div className="flex justify-between text-sm mb-2">
                            <div>
                                ...
                                <p className="text-xl font-bold">{formatCurrency(data.target.total)}</p>
                            </div>
                            <div>
                                <p className="text-gray-500">Achieved</p>
                                <p className="text-xl font-bold">{formatCurrency(data.target.achieved)}</p>
                            </div>
                            <div className="text-right">
                                ...
                                <p className="text-xl font-bold text-red-500 flex items-center justify-end gap-1">
                                    {formatCurrency(data.target.gap)} <TrendingUp className="h-4 w-4 rotate-180" />
                                </p>
                            </div>
                        </div>
                        ...
                        <p className="text-sm text-gray-500 mt-2">{data.target.percent}% of target achieved</p>
                    </div>
                        ...
                        <div className="h-[300px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={data.trend} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                                    ...
                                    <Area type="monotone" dataKey="target" name="Target" stroke="none" fill="url(#colorTarget)" />
                                    <Area type="monotone" dataKey="achieved" name="Achieved" stroke="#333333" strokeWidth={2} fill="url(#colorAchieved)" />
                                </AreaChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
                    ...
                    {/* Tactical SKU Table */}
                    <div>
                        <h3 className="font-bold text-md mb-4">Tactical SKU Performance - Gap to Goal</h3>
                        <div className="overflow-x-auto">
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        <TableHead>Product</TableHead>
                                        <TableHead className="text-right w-[150px]">Target</TableHead>
                                        <TableHead className="text-right w-[150px]">Achieved</TableHead>
                                ...
                                <TableBody>
                                    {data.skuPerformance.map((sku, idx) => (
                                        <TableRow key={idx}>
                                            <TableCell className="font-medium">{sku.product}</TableCell>
                                            <TableCell className="text-right">{formatCurrency(sku.target)}</TableCell>
                                            <TableCell className="text-right">{formatCurrency(sku.achieved)}</TableCell>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                    ...
                    <CardContent>
                        <div className="h-[350px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={data.topProducts} layout="vertical" margin={{ top: 0, right: 30, left: 70, bottom: 0 }}>
                                    <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#f0f0f0" />
                                    <XAxis type="number" hide />
                                    <YAxis dataKey="name" type="category" width={120} tick={{ fontSize: 12, fill: "#666" }} axisLine={false} tickLine={false} />
                                    <Tooltip
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="h-[350px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={data.supplierSales} margin={{ top: 20, right: 20, left: 20, bottom: 0 }}>
                                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f0f0f0" />
                                    ...
                                    <Tooltip
                                        ...
                                        formatter={(val: number) => formatCurrency(val)}
                                    />
                                    <Bar dataKey="value" fill="#000000" radius={[4, 4, 0, 0]} barSize={50} />
                                </BarChart>
                            </ResponsiveContainer>
                        </div>
                    </CardContent>
                                ...
                                <TableBody>
                                    {currentProducts.map((prod, idx) => (
                                        <TableRow key={idx}>
                                            ...
                                            <TableCell className="text-center text-sm">{prod.quantity.toLocaleString()}</TableCell>
                                ...
                                <TableBody>
                                    {currentReturns.length > 0 ? (
                                        currentReturns.map((ret, idx) => (
                                            <TableRow key={idx}>
                                                <TableCell className="font-medium text-xs">{ret.id}</TableCell>
                                                <TableCell className="text-sm">{ret.product}</TableCell>
                                                <TableCell className="text-sm text-center">{ret.date}</TableCell>
                                                <TableCell className="text-center text-sm">{ret.quantity}</TableCell>
                                                <TableCell className="text-right">
