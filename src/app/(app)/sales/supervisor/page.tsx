...
const TABS = ["Overview", "Dry Goods", "Industrial", "Mama Pina's", "Frozen Goods"];
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (!res.ok) {
                    ...
                    throw new Error(`Failed to fetch: ${res.status} ${text.slice(0, 200)}`);
                }
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col h-[350px]">
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                    <Pie data={data.salesBySupplier} innerRadius={60} outerRadius={90} paddingAngle={2} dataKey="value">
                        ...
                        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col h-[350px]">
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={data.salesBySalesman} layout="vertical" margin={{ left: 20 }}>
                                    <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#f0f0f0" />
                                    <XAxis type="number" hide />
                                    <YAxis dataKey="name" type="category" width={100} tick={{ fontSize: 11 }} axisLine={false} tickLine={false} />
                                    <Tooltip cursor={{ fill: "transparent" }} formatter={(value: number) => formatPHP(value)} />
                            ...
                            <div className="space-y-3">
                                {data.pareto?.products.slice(0, 10).map((item, idx) => (
                                    <div key={idx} className="flex items-center justify-between text-sm">
                                        <span className="text-gray-700 truncate w-2/3">{idx + 1}. {item.name}</span>
                            ...
                            <table className="w-full text-sm text-left">
                                ...
                                {data.divisionBreakdown.map((div, idx) => (
                                    <tr key={idx} className="hover:bg-gray-50">
                                        ...
                                        <td className="py-3 px-6 text-right text-gray-700">{div.goodStock.totalOutflow.toLocaleString()}</td>
                                        <td className="py-3 px-6 text-right text-red-600">{div.badStock.accumulated.toLocaleString()}</td>
